...
def execute_python(code, cus_input, user_filename):
    temp_filename = None
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".py", delete=False) as temp_file:
            ...
            # Construct the user-specific filename
            user_file_path = f"{os.path.splitext(temp_filename)[0]}_{secure_filename(user_filename)}"
            os.rename(temp_filename, user_file_path)
            
            if allowed_file(user_file_path):
                # Read the file content into a BytesIO stream
                with open(user_file_path, 'rb') as f:
                    file_stream = io.BytesIO(f.read())
                
                # Upload the file to a blob or cloud storage
                url_py_code = upload_to_blob(file_stream, user_filename)
            
        # Construct the command to run the Python script
        command = f"python {user_file_path}"
